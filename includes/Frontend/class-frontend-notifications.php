...
class SIMPNO_Frontend_Notifications {
	public function __construct() {
		...
		add_action( 'publish_post', array( $this, 'send_new_post_email' ), 10, 2 );
	...
	public function send_welcome_email( $user_id ) {
		...
		if ( ! get_option( 'notification_enable_emails_new_user_register' ) ) {
			return;
		}
	...
	public function send_new_post_email( $post_id, $post ) {
		if ( ! get_option( 'notification_enable_emails_new_post_publish' ) ) {
			return;
		}
		...
		$message        = str_replace(
			array( '{title}', '{url}' ),
			array( esc_html( $post->post_title ), esc_url( $post_url ) ),
			$email_template
		);
